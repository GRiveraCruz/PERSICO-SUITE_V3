...
// Barras simples con CSS (la Suite no usa ninguna librería de gráficas — ver
// AUDITORIA original: "no asumas tecnologías que no existen"). Cada barra es un
// <div> con altura proporcional al máximo del conjunto.
function miniBarChart(items, opts){
  opts = opts || {};
  const max = Math.max(1, ...items.map(i=>i.count));
  const h = opts.height || 130;
  return `<div style="display:flex;align-items:flex-end;gap:10px;height:${h}px;padding:0 4px">
    ${items.map(i => {
      const barH = Math.max(3, Math.round((i.count/max)*(h-28)));
      return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:100%;min-width:0">
        <div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:3px">${i.count}</div>
        <div style="width:100%;max-width:38px;height:${barH}px;background:var(--red);border-radius:4px 4px 0 0"></div>
        <div style="font-size:9px;color:var(--muted);margin-top:5px;text-align:center;line-height:1.2;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;width:100%" title="${esc(i.customer)}">${esc(i.customer)}</div>
      </div>`;
    }).join('')}
  </div>`;
}

// Pastel simple con conic-gradient de CSS — sin SVG ni canvas.
function miniPieChart(segments){
  ...
  return `<div style="display:flex;align-items:center;gap:16px">
    <div style="width:92px;height:92px;border-radius:50%;background:conic-gradient(${stops});flex-shrink:0"></div>
    <div style="display:flex;flex-direction:column;gap:6px">
      ${segments.map(s=>`<div style="display:flex;align-items:center;gap:7px;font-size:11px">
        <span style="width:10px;height:10px;border-radius:3px;background:${s.color};flex-shrink:0"></span>
        <span style="color:var(--muted2)">${esc(s.label)}</span>
        <span style="font-weight:800">${s.value}</span>
      </div>`).join('')}
    </div>
  </div>`;
}

function dashCard(title, innerHtml, extra){
  return `<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:18px 20px;${extra||''}">
    <div style="font-size:11px;font-weight:800;letter-spacing:1px;color:var(--muted2);text-transform:uppercase;margin-bottom:14px">${title}</div>
    ${innerHtml}
  </div>`;
}
function dashStat(label, value, color){
  return `<div style="display:flex;justify-content:space-between;align-items:baseline;padding:6px 0;border-bottom:1px solid var(--border);font-size:12px">
    <span style="color:var(--muted)">${label}</span>
    <span style="font-weight:800;font-size:14px;color:${color||'var(--text)'}">${value}</span>
  </div>`;
}
...
function renderGMDashboard(d){
  ...
  const salesCard = dashCard('Sales', `
    ${dashStat('Quotes registered in '+d.year, s.quotes_registered)}
    ${dashStat('Quotes sent to customer', s.quotes_sent)}
    ${s.quotes_by_customer.length ? miniBarChart(s.quotes_by_customer,{height:110}) : '<div style="font-size:11px;color:var(--muted);padding:20px 0;text-align:center">Sin cotizaciones este año</div>'}
    <div style="margin-top:14px">${miniPieChart([
      {label:'Awarded', value:s.awarded, color:'var(--green,#1f8a4c)'},
      {label:'Pending', value:s.pending, color:'var(--gold,#b3760a)'},
      {label:'Refused', value:s.refused, color:'var(--red)'},
    ])}</div>
    <div style="margin-top:14px">
      ${dashStat('POs registered in '+d.year, s.pos_registered)}
      ${dashStat('Total amount of '+d.year, fmtMoney(s.pos_total_amount), 'var(--green,#1f8a4c)')}
    </div>
  `);

  const projCard = dashCard('Projects', `
    <div style="font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:6px">${d.year}</div>
    ${dashStat('Jobs open', p.this_year.open)}
    ${dashStat('Jobs in WIP', p.this_year.wip)}
    ${dashStat('Jobs closed', p.this_year.closed)}
    ${p.this_year.by_customer.length ? miniBarChart(p.this_year.by_customer,{height:110}) : ''}
    <div style="font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;margin:16px 0 6px">${d.year-1}</div>
    ${dashStat('Jobs open', p.last_year.open)}
    ${dashStat('Jobs in WIP', p.last_year.wip)}
    ${dashStat('Jobs closed', p.last_year.closed)}
    ${p.last_year.by_customer.length ? miniBarChart(p.last_year.by_customer,{height:110}) : '<div style="font-size:11px;color:var(--muted);padding:16px 0;text-align:center">Sin jobs registrados</div>'}
  `);

  const costCard = dashCard(`Cost Control (${d.year})`, `
    ...
    ${dashStat('Gross margin', fmtMoney(cc.gross_margin), cc.gross_margin>=0?'var(--green,#1f8a4c)':'var(--red)')}
    <div style="margin-top:14px;max-height:320px;overflow-y:auto;border:1px solid var(--border);border-radius:6px">
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead style="position:sticky;top:0;background:var(--panel)">
          <tr style="text-align:left">
            <th style="padding:7px 10px;border-bottom:1px solid var(--border)">Job</th>
            <th style="padding:7px 10px;border-bottom:1px solid var(--border);text-align:right">Revenue</th>
            <th style="padding:7px 10px;border-bottom:1px solid var(--border);text-align:right">Cost</th>
            <th style="padding:7px 10px;border-bottom:1px solid var(--border);text-align:right">Result</th>
          </tr>
        </thead>
        <tbody>
          ${cc.jobs.map(j=>`<tr>
            <td style="padding:6px 10px;border-bottom:1px solid var(--border);font-weight:700">${esc(j.job_number)}</td>
            <td style="padding:6px 10px;border-bottom:1px solid var(--border);text-align:right">${fmtMoney(j.revenue)}</td>
            <td style="padding:6px 10px;border-bottom:1px solid var(--border);text-align:right">${fmtMoney(j.cost)}</td>
            <td style="padding:6px 10px;border-bottom:1px solid var(--border);text-align:right;font-weight:700;color:${j.result>=0?'var(--green,#1f8a4c)':'var(--red)'}">${fmtMoney(j.result)}</td>
  ...
  wrap.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px">
      <div>
        <div style="font-size:9px;font-weight:700;letter-spacing:2px;color:var(--muted);text-transform:uppercase">General Management</div>
        <div style="font-size:18px;font-weight:900;letter-spacing:1px;color:var(--text)">Dashboard</div>
      </div>
      <select onchange="loadGMDashboard(this.value)" style="padding:7px 12px;border-radius:6px;border:1px solid var(--border2);background:var(--inp);font-size:12px">
