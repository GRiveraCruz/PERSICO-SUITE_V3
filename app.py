...
def api_dashboard_general_management():
    ...
    try:
        ...
        # ── VENTAS: Purchase Orders (todas las de la Suite, no por job) ──
        po_year_records = po_load(year)
        po_active = [p for p in po_year_records if (p.get("estatus") or "") != "Cancelada"]
        fx_all = fx_load_all()
        pos_total_amount = round(sum(_po_usd(p, fx_all) for p in po_active), 2)
